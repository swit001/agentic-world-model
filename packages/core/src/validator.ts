...
export function validateWorld(world: World): ValidationResult {
  const valid = validate(world) as boolean;
  ...
  if (valid || schemaErrors.length === 0) {
    for (const [transName, trans] of Object.entries(world.transitions)) {
      ...
      if (!entity) {
        semanticErrors.push(
          `Transition "${transName}" references unknown entity "${trans.entity}"`
        );
        continue;
      }
      if (!entity.states.includes(trans.from)) {
        semanticErrors.push(
          `Transition "${transName}" has unknown from-state "${trans.from}" for entity "${trans.entity}"`
        );
      }
      if (!entity.states.includes(trans.to)) {
        semanticErrors.push(
          `Transition "${transName}" has unknown to-state "${trans.to}" for entity "${trans.entity}"`
        );
      }
    }
  }
